...
const T = {
  ru: {
    ...
    walk_s6:      "Сначала выберите язык интерфейса и язык, на который хотите переводить текст — все метки и переводы подстроятся под ваш выбор.",
    walk_s7:      "Затем загрузите свой API-ключ от OpenAI, Anthropic, Google или xAI — его можно бесплатно получить на сайте соответствующего провайдера. Ключ хранится только в вашем браузере.",
  ...
  en: {
    ...
    walk_s6:      "First, choose your interface language and the language you want to translate into — all labels and translations adapt to your pick.",
    walk_s7:      "Then upload your API key from OpenAI, Anthropic, Google or xAI — you can get one for free on the provider's website. The key stays in your browser only.",
  ...
  es: {
    ...
    walk_s6:      "Primero elige el idioma de la interfaz y el idioma al que quieres traducir — todas las etiquetas y traducciones se adaptan a tu elección.",
    walk_s7:      "Luego sube tu clave API de OpenAI, Anthropic, Google o xAI — puedes obtenerla gratis en el sitio web del proveedor correspondiente. La clave se guarda solo en tu navegador.",
  ...
  zh: {
    ...
    walk_s6:      "首先选择界面语言以及您希望翻译的目标语言——所有标签和翻译都会根据您的选择自动调整。",
    walk_s7:      "然后上传您来自 OpenAI、Anthropic、Google 或 xAI 的 API 密钥——可在对应提供商的官网免费获取。密钥仅保存在您的浏览器中。",
