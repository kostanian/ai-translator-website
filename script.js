...
const T = {
  ru: {
    ...
    walk_s2:      "Нажмите кнопку — откроется попап с переводом и тремя кнопками: 💡 объяснение, ➕ в словарь, ⧉ копировать.",
    ...
    walk_s6:      "Откройте расширение и выберите два языка: сверху — язык интерфейса (на котором будут подписи и объяснения), снизу — язык, на который хотите переводить выделенный текст. Все подсказки тут же подстроятся.",
    walk_s7:      "Перейдите в настройки, выберите своего ИИ-провайдера (OpenAI, Anthropic, Google или xAI), вставьте API-ключ в поле и сохраните. Ключ можно бесплатно получить на сайте провайдера — он хранится только в вашем браузере.",
  ...
  en: {
    ...
    walk_s2:      "Click the button — a popup opens with the translation and three actions: 💡 explain, ➕ save to dictionary, ⧉ copy.",
    ...
    walk_s6:      "Open the extension and pick two languages: the top one is the interface language (labels and explanations), the bottom one is the language you want to translate the selected text into. Everything adjusts instantly.",
    walk_s7:      "Go to settings, pick your AI provider (OpenAI, Anthropic, Google or xAI), paste your API key into the field and save. You can get the key for free on the provider's website — it is stored in your browser only.",
  ...
  es: {
    ...
    walk_s2:      "Haz clic — se abre un popup con la traducción y tres acciones: 💡 explicar, ➕ diccionario, ⧉ copiar.",
    ...
    walk_s6:      "Abre la extensión y elige dos idiomas: arriba, el idioma de la interfaz (etiquetas y explicaciones); abajo, el idioma al que quieres traducir el texto seleccionado. Todo se ajusta al instante.",
    walk_s7:      "Ve a ajustes, elige tu proveedor de IA (OpenAI, Anthropic, Google o xAI), pega tu clave API en el campo y guarda. Puedes obtener la clave gratis en el sitio del proveedor — se guarda solo en tu navegador.",
  ...
  zh: {
    ...
    walk_s2:      "点击按钮——弹出翻译窗口，含三个操作：💡 解释、➕ 词典、⧉ 复制。",
    walk_s3:      "将鼠标悬停在灯泡上——提示「在上下文中解释」。点击启动 AI 句子级分析。",
    walk_s4:      "结构化解析：原形、使用的形式（带示例）、所在短语、此处含义，以及通俗语言的原因说明。",
    walk_s6:      "打开扩展并选择两种语言：上方为界面语言（用于标签和解释），下方为您希望将所选文本翻译成的目标语言。所有内容会立即调整。",
    walk_s7:      "进入设置，选择您的 AI 提供商（OpenAI、Anthropic、Google 或 xAI），将 API 密钥粘贴到字段中并保存。密钥可在对应提供商的官网免费获取——仅保存在您的浏览器中。",
